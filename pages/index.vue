...
const propertiesConfig = reactive<PropertiesConfig>({
  isDark: true,
  properties: [
    {
      subset: ['customColors', 'palettes'],
    },
    {
      suffix: '-rgb',
...
const properties = computed(() => {
  return cssPropertiesFromTheme(theme.value, propertiesConfig);
});

const textContent = computed(() => toStyleTextContent(properties.value));
...
<template>
  <div>
    ...
    <div class="grid grid-cols-2">
      ...
      <!--      <div>-->
      <!--        <h2>Input</h2>-->
      <!--        <pre><code class="block bg-surface-level-1 p-4 rounded-xl">{{ theme }}</code></pre>-->
      <!--      </div>-->
      <div>
        ...
        <div v-for="(customColor, i) in themeConfig.customColors" :key="i" class="flex flex-col">
          <pre class="hidden">{{
            theme.customColors.find((item) => item.color.name === customColor.name)
          }}</pre>
          <div class="flex">
